...
@UseTestDataFrom(value = Constants.BMO_RESOURCES_PREFIX + "Test_LessEntitlements_AnalystChoiceWidget.csv", separator=',')

public class Test_LessEntitlements_AnalystChoiceWidget{
...
//			businessRulesWidgetsSteps.clickOnSelectedPage(page,pivotPoint, pivotName, continent, otherRating,analysis,news,itemNumber, contentType,subContextType, serviceLeftHandFilter, geographyLeftHandFilter,subPageName,countryId, articleTypeNumber, articleId,reportIssueNumber);
		 ...
		 if(page.contains("Hot Topics")){
				...
				if(foundHotTopic){
					...
					businessRulesWidgetsSteps.clickFirstArticleAndVerifyPageNotFound(widgetContainer, savedTitle, widgetId, contentType);
				}
			}
				...
				businessRulesWidgetsSteps.clickFirstArticleAndVerifyPageNotFound(widgetContainerSAR, savedTitle, widgetId, contentType);
			}
